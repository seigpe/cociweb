...
POD_INSTANCE_NAME=`/home/seigpe/proyectos/coci/oc get pods -l "name=${POD_NAME:-django-frontend}"  -t "{{ with index .items ${POD_INDEX:-0} }}{{ .metadata.name }}{{ end }}"`

# Run command in a container of the specified pod:
/home/seigpe/proyectos/coci/oc exec -p "$POD_INSTANCE_NAME" -it -- bash -c "${@:-echo}"
